...
```{r}
str(coffee_cancer_dag)
class(coffee_cancer_dag)

coffee_cancer_dag |>
  tidy_dagitty()
# Gives you a nice tidy tibble with a row for each edge

coffee_cancer_dag |>
  tidy_dagitty() |>  
  ggdag_paths() +  # for paths that are open
  theme_void() +
...
ggdag_adjustment_set(coffee_cancer_dag,
                     use_labels=TRUE, label=label,
                     use_text=FALSE)
dagitty::adjustmentSets(coffee_cancer_dag)

coffee + addictive ~ cancer # one possible formula
coffee + smoking ~ cancer # another possible formula; closes off the same path

dagitty::adjustmentSets(coffee_cancer_dag, type="all")
ggdag_adjustment_set(coffee_cancer_dag,
                     use_labels=TRUE, label=label,
                     use_text=FALSE, type="all")

coffee + smoking + addictive ~ cancer
```

Why not adjust for both together? the `dagitty::adjustmentSets()` gives you a **minimal** adjustment set. But you can also ask for the full set of adjustments and one will include both.
